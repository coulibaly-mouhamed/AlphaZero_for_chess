...
'''
Script to encode or decode an chess action to a number and also 
a chess board state to a vector encoded state in numpy array
'''
    ...
    if piece in ["p", "P"] and (x == 0 or x == 7) and underpromote != None: # underpromotions
        ...
        if abs(dx) == 1 and dy == 1:
            ...
            if underpromote == "bishop":
                idx = 72
    encoded[i,j,idx] = 1
    encoded = encoded.reshape(-1) # flatten
...
if __name__ == '__main__':
    board = chessboard()
    #Print the state of the board in the console
    print('Encoded board....\n')
    encoded_board = encode_board(board)
    print(encoded_board[:,:,15])
